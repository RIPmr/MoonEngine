...
namespace ImGui {
	...
	namespace {
		...
		inline void AddTextVertical(ImDrawList *DrawList, const char *text, ImVec2 pos, ImU32 text_color) {
			...
			while ((c = *text++)) {
				...
				DrawList->PrimQuadUV(
					pos + ImVec2(glyph->Y0 * scale, -glyph->X0 * scale),
					pos + ImVec2(glyph->Y0 * scale, -glyph->X1 * scale),
					pos + ImVec2(glyph->Y1 * scale, -glyph->X1 * scale),
					pos + ImVec2(glyph->Y1 * scale, -glyph->X0 * scale),
	...
	bool Plot(const char *label_id, PlotInterface &plot, std::vector<PlotItem> &items, const ImVec2 &size) {
		return Plot(label_id, &plot, items.size() > 0 ? &items[0] : nullptr, items.size(), size);
	}
	...
	bool Plot(const char *label_id, PlotInterface *plot_ptr, PlotItem *items, int nItems, const ImVec2 &size) {
		...
		g_plot_hovered = xAxisRegion_hovered || yAxisRegion_hovered;
	...
	bool IsPlotFrameHovered() { return g_plot_hovered; }
