...
namespace ImGui {
	static void DrawGradientBar(ImGradient* gradient, struct ImVec2 const & bar_pos,
		float maxWidth, float height) {
		ImVec4 colorA = { 1,1,1,1 }; ImVec4 colorB = { 1,1,1,1 };
		float prevX = bar_pos.x;
		float barBottom = bar_pos.y + height;
		ImGradientMark* prevMark = nullptr;
		ImDrawList* draw_list = ImGui::GetWindowDrawList();

		auto col = ImGui::GetStyle().Colors[ImGuiCol_Border];
		draw_list->AddRectFilled(
			ImVec2(bar_pos.x - 1, bar_pos.y - 1),
		...
		for (auto markIt = gradient->getMarks().begin();
			...
			if (mark->position > 0.0) {
				draw_list->AddRectFilledMultiColor(
					ImVec2(from, bar_pos.y), ImVec2(to, barBottom),
	...
	static void DrawGradientMarks(ImGradient* gradient, ImGradientMark* & draggingMark,
		...
		ImGradientMark* prevMark = nullptr;
		ImDrawList* draw_list = ImGui::GetWindowDrawList();
		ImU32 colorAU32 = 0; ImU32 colorBU32 = 0;
		...
		for (auto markIt = gradient->getMarks().begin(); 
			...
			auto col = ImGui::GetStyle().Colors[ImGuiCol_Border];
			auto col32 = IM_COL32(col.x * 255, col.y * 255, col.z * 255, col.w * 255);
			draw_list->AddTriangleFilled(
				ImVec2(to, bar_pos.y + (height - 2)),
				ImVec2(to - 2, barBottom),
				ImVec2(to + 2, barBottom),
				col32
			);

			draw_list->AddRectFilled(
				ImVec2(to - 3, barBottom),
				ImVec2(to + 3, bar_pos.y + (height + 10)),
				col32, 1.0f, 1.0f
			);
			...
			draw_list->AddRectFilledMultiColor(
				ImVec2(to - 2, bar_pos.y + (height + 1)),
				ImVec2(to + 2, bar_pos.y + (height + 9)),
				colorBU32, colorBU32, colorBU32, colorBU32
			);

			ImGui::SetCursorScreenPos(ImVec2(to - 3, barBottom));
			ImGui::InvisibleButton("mark", ImVec2(6, 12));
	...
	bool GradientEditor(ImGradient* gradient, ImGradientMark* & draggingMark, ImGradientMark* & selectedMark) {
		...
		ImVec2 bar_pos = ImGui::GetCursorScreenPos(); bar_pos.x += 10;
		...
		if (selectedMark) {
			bool colorModified = ImGui::ColorEdit3("Gradient", selectedMark->color);
