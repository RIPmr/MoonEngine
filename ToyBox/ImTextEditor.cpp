...
TextEditor::TextEditor()
	...
	, mColorRangeMin(0)
	, mColorRangeMax(0)
	, mSelectionMode(SelectionMode::sel_normal)
	, mCheckComments(true)
	, mLastClick(-1.0f)
	...
	if (ImGui::IsWindowHovered()) {
		if (!shift && !alt) {
			...
			*/
			...
			if (tripleClick) {
				if (!ctrl) {
					...
					mSelectionMode = SelectionMode::sel_line;
			...
			*/
				...
				if (!ctrl) {
					mState.mCursorPosition = mInteractiveStart = mInteractiveEnd = ScreenPosToCoordinates(ImGui::GetMousePos());
					if (mSelectionMode == SelectionMode::sel_line)
						mSelectionMode = SelectionMode::sel_normal;
					else
						mSelectionMode = SelectionMode::sel_word;
					SetSelection(mInteractiveStart, mInteractiveEnd, mSelectionMode);
				}
			...
			if (!id.empty()) {
				...
				if (it != mLanguageDefinition.mIdentifiers.end()) {
					...
					if (pi != mLanguageDefinition.mPreprocIdentifiers.end()) {
						if (!pi->second.mDeclaration.empty()) {
							ImGui::BeginTooltip();
...
void TextEditor::SetSelection(const Coordinates & aStart, const Coordinates & aEnd, SelectionMode aMode) {
	...
	switch (aMode) {
	case TextEditor::SelectionMode::sel_normal:
		break;
	case TextEditor::SelectionMode::sel_word:
	{
	...
	case TextEditor::SelectionMode::sel_line:
	{
...
void TextEditor::MoveLeft(int aAmount, bool aSelect, bool aWordMode) {
	...
	if (aSelect) {
		...
		mInteractiveStart = mInteractiveEnd = mState.mCursorPosition;
	SetSelection(mInteractiveStart, mInteractiveEnd, aSelect && aWordMode ? SelectionMode::sel_word : SelectionMode::sel_normal);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::GLSL() {
	...
	if (!inited) {
		static const char* const keywords[] = {
			...
			"discard", "mat2", "mat3", "mat4","vec2", "vec3", "vec4", "ivec2", "ivec3", "ivec4", "bvec2", "bvec3", "bvec4",
			"template", "this", "packed", "flat", "half", "fixed", "superp",
		...
		static const char* const identifiers[] = {
			...
			"ispunct", "isspace", "isupper", "kbhit", "log10", "log2", "log", "memcmp", "modf", "pow", "putchar", "putenv", 
